...
### なぜ VM ではなく LXC + Docker Tailscale か

常設サーバ側の AlmaLinux は VM ではなく LXC コンテナで動かす。リソースオーバーヘッドを最小限にするため。

LXC 上で Tailscale を動かすには TUN デバイスの問題がある（LXC はデフォルトで `/dev/net/tun` にアクセスできない）が、Tailscale を Docker コンテナとして動かし `TS_USERSPACE=true`（userspace networking mode）を使うことで回避する。

userspace mode の制約（subnet routing 不可、パフォーマンス低下）は今回の用途では影響しない:
- subnet routing → 不要。LXC 自身のサービスにアクセスするだけ
- パフォーマンス → Mattermost のテキストと Ollama API コール程度なので問題なし

Tailscale も含めて全サービスが docker-compose.yml に収まるため、環境依存性が低くポータビリティも高い。
...
常設サーバ側は Tailscale を Docker コンテナとして動かし、`network_mode: "service:tailscale"` で他のコンテナがそのネットワークスタックを共有する。
...
CoreDNS も同じ docker-compose 内で動かす。Tailscale の MagicDNS でも良いが、自前で管理したい場合は CoreDNS が柔軟。
...
### 常設サーバ（Proxmox + AlmaLinux LXC）

Mattermost, CoreDNS, Tailscale, Bot 等の常駐サービスを集約。全て Docker コンテナとして動かす。

1. Proxmox 上に AlmaLinux LXC を作成（nesting=1 を有効化）
...
- **LXC + Docker の安定性**。nesting=1 での Docker 動作に問題が出ないか
...
4. 常設サーバに AlmaLinux LXC を作成、Docker 環境を構築
5. Tailscale + Mattermost Team Edition + CoreDNS をコンテナで立てる
